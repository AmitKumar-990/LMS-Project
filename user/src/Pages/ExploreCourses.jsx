...
export default function ExploreCourses() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ensure consistent fields (price number, difficulty optional)
        ...
        // set dynamic priceRange bounds based on data
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter(c => {
      ...
      if (difficulty !== "All" && (c.difficulty || "Beginner") !== difficulty) return false;
      if (c.price < minPrice || c.price > maxPrice) return false;
      if (query) {
        const q = query.toLowerCase();
        if (!(`${c.title} ${c.description || ""} ${c.instructor?.name || ""}`.toLowerCase().includes(q))) return false;
      }
      return true;
    });
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="bg-white rounded-xl p-4 shadow flex flex-col md:flex-row gap-3 items-center">
          ...
          <div className="flex-1">
            {/* If you installed react-debounce-input you can use it; else replace with regular input */}
            <input
              value={query}
          ...
          <div className="flex gap-2 items-center overflow-auto">
            {categories.map(cat => (
              <button
                ...
                className={`px-3 py-2 rounded-full text-sm ${category === cat ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
              >
                {cat}
              </button>
            ))}
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="text-sm text-gray-500">Showing {paginated.length} of {filtered.length}</div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {paginated.map((course, idx) => (
            <div
              key={course._id}
              className={`transform transition duration-400 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
